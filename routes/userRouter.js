...
const { register, login, loggedInUser,getUser,getAllUsers,updateUser,forgotYourPassword,verify,changePass } = require("../controllers/userController");
...
userRouter.post("/forgotYourPassword", async(req,res) => {
  try {
    // if(!req.isAuth) throw new Error("Unauthenticated");
    const data = await forgotYourPassword(req);
    res.send(data);
  } catch (error) {
    console.log(error);
    res.send({ err: error.message });
